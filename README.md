...
| Grounding with Bing Search resource (optional) | Create an Azure resource of type **Grounding with Bing Search** (separate paid Bing offering, NOT the deprecated Bing Search v7). Then add it as a Foundry connection — note the connection name for `BING_CONNECTION_NAME` |
...
| `BING_CONNECTION_NAME` | Optional. Foundry connection name for the Grounding with Bing Search resource. Leave empty to build the toolbox without the Bing tool. |
...
3. If `BING_CONNECTION_NAME` is set, also resolves the Bing connection and adds a `BingGroundingTool` (external grounding).
