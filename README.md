...
This spike's toolbox bundles two grounding sources:

- **`meeting-mins-ai-search`** — internal MTN executive board meeting
  minutes, decisions, action items, owners (Azure AI Search over
  `meeting-mins-index`).
- **`bing-grounding`** — external public web for news, share prices,
  competitive intelligence, telco industry highlights, regulatory updates
  (Grounding with Bing Search).

The agent's system prompt teaches `gpt-5.1` to pick the right tool — or call
both and synthesise — based on the question.
...
flowchart LR
    ...
    subgraph foundry["Azure AI Foundry — project: foundry-toolbox"]
        ...
        toolbox["Toolbox: meeting-mins-toolbox v2<br/>(MCP endpoint)"]
        searchconn["Connection: aisearchcorepo7u7j<br/>(Entra ID auth)"]
        bingconn["Connection: bing-grounding<br/>(API key)"]
    end

    subgraph azsearch["Azure AI Search"]
        index["Index: meeting-mins-index<br/>(internal: board minutes)"]
    end

    subgraph bing["Grounding with Bing Search"]
        web["Public web<br/>(news, share prices, telco industry)"]
    end

    agent -- "1. chat (Responses API)" --> model
    model -- "2. tool call planning" --> agent
    agent -- "3. MCP call" --> toolbox
    toolbox -- "4a. AI Search (Foundry MI)" --> searchconn
    toolbox -- "4b. Bing grounding (connection key)" --> bingconn
    searchconn --> index
    bingconn --> web
    index -. "5a. internal hits" .-> toolbox
    web -. "5b. external hits" .-> toolbox
    toolbox -. "6. tool result(s)" .-> agent
    agent -- "7. synthesised answer + citations" --> model
```
...
| Azure AI Search connection | Inside the Foundry project, name e.g. `aisearchcorepo7u7j`, **Entra ID** auth, pointing at the `ai-search-core` search service |
| Search index | `meeting-mins-index` exists in `ai-search-core` |
| Grounding with Bing Search resource | Create an Azure resource of type **Grounding with Bing Search** and add it as a Foundry connection. You do NOT need to reference the connection by name in code — Foundry routes the toolbox's `web_search` tool to it automatically. |
...
| `ENABLE_WEB_SEARCH` | `true` to include the Bing-backed web search tool in the toolbox, `false` to omit it. Default `true`. |
...
1. Resolves the Foundry AI Search connection by name → gets the full ARM resource id.
2. Builds an `AzureAISearchTool` (internal grounding over `meeting-mins-index`).
3. If `ENABLE_WEB_SEARCH` is true (default), adds a `WebSearchTool` (external grounding). Foundry routes this to the project's Grounding-with-Bing connection automatically — no connection id needed in code.
4. Calls `client.beta.toolboxes.create_version(name=..., tools=[...])`.
5. Constructs the MCP endpoint URL from the project endpoint, toolbox name, and version (the SDK doesn't return it directly).
...
| `bing_grounding` tool type rejected by toolbox | Toolboxes only accept `web_search` (not `bing_grounding`). Foundry auto-binds it to the project's Grounding-with-Bing connection — no connection id needed in code |
